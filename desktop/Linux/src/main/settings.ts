...
// here. The freedesktop.org autostart spec instead reads ~/.config/autostart/*.desktop on
...
// Apply the launch-at-login preference to the OS by writing or removing the autostart
// .desktop file. Best-effort: a failure here must not break persisting settings.json.
...
class SettingsStore extends EventEmitter {
  ...
  set(partial: Partial<AppSettings>): AppSettings {
    ...
    if (partial.launchAtLogin !== undefined && partial.launchAtLogin !== before.launchAtLogin) {
      applyLaunchAtLogin(this.data.launchAtLogin)
    }
    try {
